        ...
        for i in trange(len(imgs)):
            ...
            with torch.no_grad():
                ...
                generated_output_qformer = self.class_generator.generate_1n_avg_qformer(
                    img_idd =  torch.tensor(img_id)[batch_indices].tolist(),
                    pixel_values=torch.stack(images_batch)[batch_indices].cuda().to(torch.float16),
                    qformer_input_ids=torch.cat(qformer_input_ids_batch)[batch_indices].cuda(),
                    qformer_attention_mask=torch.cat(qformer_attention_mask_batch)[batch_indices].cuda(),
                    input_ids=torch.cat(input_ids_batch)[batch_indices].cuda(),
                    attention_mask=torch.cat(attention_mask_batch)[batch_indices].cuda(),
                    cache_image_embeds=True,
                    segmentation_mask=torch.cat(segmentation_masks_batch)[batch_indices].cuda(),
                    input_context_mask=torch.cat(context_mask_batch)[batch_indices].cuda(),
                    dataset_type="any",
                    max_new_tokens=16,
                    num_beams=1,
