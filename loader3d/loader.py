...
import pdb
import torch
from loader3d.utils.rotation import convert_angle_axis_to_matrix3
from loader3d.utils.taxonomy import class_names, ARKitDatasetConfig
import loader3d.utils.pc_utils as pc_utils


def align_pixel(mapping, a, b, c, d):
...
class ScannetReader(object):
    def __init__(
        ...
        self.global_intrinsic = np.loadtxt('/home/phucnda/applied_cv/Dataset/ScannetV2/ScannetV2_2D_5interval/trainval/intrinsic_depth.txt')
        self.poses = {}
        if os.path.exists(os.path.join(self.root_path, "intrinsic.txt")):
            intrinsic_file = os.path.join(self.root_path, "intrinsic.txt")
        else:
            intrinsic_file = os.path.join(self.root_path, "intrinsic/intrinsic_depth.txt")
