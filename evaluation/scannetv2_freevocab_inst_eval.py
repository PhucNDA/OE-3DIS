...
from dataset3d.scannet200 import \
...
'''
N prediction, M groundtruth
Cost Matrix C(N, M): Cost between prediction n and g.truth m (IoU & similarity(sentence_transformer))
setA: pair <i,j> predicted & gt || setB unmatch prediction or gt
TruePostive   : Hungarian match (pair<i,j>: i will be true positive of that gt j instance)
FalsePositive : The remaining u predictions with C[u,j] larger than the threshold
FalseNegative : 
'''


class ScanNetFreeVocabEval(object):
