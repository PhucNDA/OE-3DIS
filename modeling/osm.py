...
class OmniScientModel(nn.Module):
    ...
    def sliding_window_vit_forward(self,
        ...
        for h_idx in range(self.h_grids):
            for w_idx in range(self.w_grids):
                ...
                cur_pixel_values = pixel_values[..., y1:y2, x1:x2] # (batch, 3, 224, 224)

                target_dtype = self.base_model.vision_model.embeddings.patch_embedding.weight.dtype
                patch_embeds = self.base_model.vision_model.embeddings.patch_embedding(cur_pixel_values)  # (batch, 1408, 16, 16)
                patch_embeds = patch_embeds.flatten(2).transpose(1, 2) # (batch, 256, 1408)

                class_embeds = self.base_model.vision_model.embeddings.class_embedding.expand(batch_size, 1, -1).to(target_dtype) # (batch, 1, 1408)
                embeddings = torch.cat([class_embeds, patch_embeds], dim=1) # (batch, 257, 1408)
                position_embedding = self.base_model.vision_model.embeddings.position_embedding.to(target_dtype) # (batch, 257, 1408)
                hidden_states = embeddings + position_embedding
                last_hidden_state = self.base_model.vision_model.encoder(
                                    inputs_embeds=hidden_states)[0] # (batch, 257, 1408)
                last_hidden_state = self.base_model.vision_model.post_layernorm(last_hidden_state)
                last_hidden_state, cls_tokens = last_hidden_state[:, 1:], last_hidden_state[:, 0:1] # (batch, 256, 1408) # (batch, 1, 1408)
    ...
    def generate_attention_mask(self, image_embeds, segmentation_mask, context_mask):
        ...
        context_attn_masks = F.interpolate(context_mask.to(image_embeds.dtype),
                                               size=(self.input_size//self.backbone_output_stride, self.input_size//self.backbone_output_stride), mode='nearest').reshape(batch_size, -1).to(torch.long)
        # pre-pend ones for cls tokens. Carefully look at here
    ...
    def maskqformer_encoding(self, image_embeds, image_attention_mask, image_context_attention_mask, dataset_type, qformer_input_ids, qformer_attention_mask):
        '''
        image_embeds: [b, 6425, 1408]
        image_attention_mask: [b, 6425] (0,1)
        image_context_attention_mask: [b,6425] (0,1) 
        qformer_input_ids: [b, 12] # Maybe for the LLM
        qformer_attention_mask: [b,12]
        '''
    ...
    def llm_decoding(self, qformer_output, dataset_type, input_ids, attention_mask, is_generation, **generate_kwargs):
        ...
        if is_generation:
            self.base_model.language_model.config.pad_token_id = 0
            with torch.no_grad():
    ...
    def generate(
        ...
        **generate_kwargs,
        ...
        ### Two pointer fast calculation
        intervals = []
        current_interval_start = 0
        for i in range(1, pixel_values.shape[0]):
            if (img_idd[i - 1] != img_idd[i]):
                current_interval_end = i - 1
                intervals.append((current_interval_start, current_interval_end))
                current_interval_start = i

        current_interval_end = pixel_values.shape[0] - 1
        intervals.append((current_interval_start, current_interval_end))

        image_embeds = torch.zeros((pixel_values.shape[0], 6425, 1408), dtype=torch.float16).to('cuda:0')
        temporary_img = torch.zeros((len(intervals), 3, 1120, 1120), dtype=torch.float16).to('cuda:0')
        
        for i in range(len(intervals)):
            temporary_img[i] = pixel_values[intervals[i][0]]
        
        tmp = self.sliding_window_vit_forward(temporary_img)
        tmp += self.window_pos_embed
        
        for i, (start, end) in enumerate(intervals):
            image_embeds[start:end + 1] = tmp[i]
        ...
        del image_attention_mask, image_context_attention_mask, qformer_output, image_embeds
        torch.cuda.empty_cache()
        return llm_outputs

    @torch.no_grad()
    def generate_1n_avg_qformer(
        self,
        img_idd,
        pixel_values,
        qformer_input_ids = None,
        ...
        **generate_kwargs,
        ...
        # Generate Qformer embedding from the prior generate() function
        ...
        qformer_output = self.maskqformer_encoding(image_embeds, image_attention_mask, image_context_attention_mask, dataset_type, qformer_input_ids, qformer_attention_mask)
        del image_attention_mask, image_context_attention_mask, image_embeds
        torch.cuda.empty_cache()
        return qformer_output    

    @torch.no_grad()
    def generate_1n_avg_language(
        self,
        qformer_output,
        input_ids = None,
        attention_mask = None,
        ...
        **generate_kwargs,
        ...
        ### Average qformer_output
        qformer_output = torch.mean(qformer_output, 0, True)
        llm_outputs = self.llm_decoding(qformer_output, dataset_type, input_ids, attention_mask, is_generation=True, **generate_kwargs)
